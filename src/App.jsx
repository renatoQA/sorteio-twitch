...
const defaultState = { viewers: {}, liveActive: false, liveDate: null, streamerUnlocked: false, winner: null };

export default function App() {
  const [state, setState] = useState(() => JSON.parse(localStorage.getItem(STORAGE_KEY) || "null") || defaultState);
  ...
  function flash(msg, color = "#9146FF") {
  ...
  function doRegister() {
    ...
    if (state.viewers[n]) return flash("Nick já cadastrado!", "#FF4747");
    const newViewer = { nick: n, code: uid(), sessions: [], checkedInToday: false };
    setState(s => ({ ...s, viewers: { ...s.viewers, [n]: newViewer } }));
    setPreview(newViewer);
    setNick("");
    flash("Cadastrado! ✅", "#00C853");
  }
  ...
  function doCheckin() {
    ...
    if (!v) return flash("Nick não encontrado!", "#FF4747");
    if (!state.liveActive) return flash("Nenhuma live ativa agora.", "#FF4747");
    if (v.checkedInToday) return flash("Você já fez check-in hoje!", "#FFB347");
    setState(s => ({
      ...s, viewers: {
        ...s.viewers, [n]: { ...v, sessions: [...v.sessions, { date: s.liveDate, minutes: 0 }], checkedInToday: true }
      }
    }));
    setNickCI("");
    flash("Check-in feito! ✅", "#00C853");
  }

  function toggleLive() {
    if (!state.liveActive) {
      const viewers = {};
      Object.values(state.viewers).forEach(v => { viewers[v.nick] = { ...v, checkedInToday: false }; });
      setState(s => ({ ...s, liveActive: true, liveDate: today(), viewers }));
      flash("Live aberta! Viewers podem fazer check-in.", "#00C853");
    } else {
      setState(s => ({ ...s, liveActive: false }));
      flash("Live encerrada.");
    }
  }

  function addTime(nick, minutes) {
    const v = state.viewers[nick];
    if (!v) return;
    const sessions = [...v.sessions];
    const idx = sessions.findLastIndex(s => s.date === state.liveDate);
    if (idx === -1) return;
    sessions[idx] = { ...sessions[idx], minutes: sessions[idx].minutes + minutes };
    setState(s => ({ ...s, viewers: { ...s.viewers, [nick]: { ...v, sessions } } }));
  }

  function drawWinner() {
    const elig = Object.values(state.viewers).filter(isEligible);
    if (!elig.length) return flash("Nenhum elegível ainda.", "#FF4747");
    const winner = elig[Math.floor(Math.random() * elig.length)];
    setState(s => ({ ...s, winner }));
    flash(`🎉 Vencedor: ${winner.nick}!`);
  }

  function unlockStreamer() {
    if (pass === PASS) { setState(s => ({ ...s, streamerUnlocked: true })); setPass(""); }
    else flash("Senha incorreta.", "#FF4747");
  }

  function resetAll() {
    if (!window.confirm("Zerar tudo? Não tem como desfazer.")) return;
    setState({ ...defaultState, streamerUnlocked: true });
    flash("Sistema zerado.");
  }

  const vList = Object.values(state.viewers).sort((a, b) => totalScore(b) - totalScore(a));
  ...
  const s = {
    ...
    tabs: { display: "flex", borderBottom: "1px solid #26262C", padding: "0 16px", background: "#18181B" },
    tab: (active) => ({ padding: "11px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer", color: active ? "#9146FF" : "#ADADB8", borderBottom: active ? "2px solid #9146FF" : "2px solid transparent", background: "none", border: "none", borderBottom: active ? "2px solid #9146FF" : "2px solid transparent" }),
    body: { padding: 16 },
    ...
    btn: (bg = "#9146FF") => ({ background: bg, color: "#fff", border: "none", borderRadius: 8, padding: "9px 16px", fontWeight: 700, cursor: "pointer", fontSize: 13 }),
    ...
    tip: { background: "#9146FF18", border: "1px solid #9146FF44", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "#C9A7FF", marginBottom: 12 },
  ...
  function ViewerStatus({ v }) {
    ...
    const sorted = Object.values(state.viewers).sort((a, b) => totalScore(b) - totalScore(a));
    const rank = sorted.findIndex(x => x.nick === v.nick) + 1;
    return (
      <div style={{ ...s.card, borderColor: ok ? "#00C85344" : "#9146FF44" }}>
        ...
        <div style={s.grid2}>
          <div>
            ...
            <div style={{ fontWeight: 700, fontSize: 20, color: days >= MIN_DAYS ? "#00C853" : "#9146FF" }}>{days}<span style={{ fontSize: 12, color: "#ADADB8" }}>/{MIN_DAYS}</span></div>
            <div style={s.prog}><div style={{ height: "100%", borderRadius: 20, width: `${Math.min(100, days / MIN_DAYS * 100)}%`, background: days >= MIN_DAYS ? "#00C853" : "#9146FF", transition: "width .4s" }} /></div>
          </div>
          <div>
            <span style={s.label}>minutos</span>
            <div style={{ fontWeight: 700, fontSize: 20, color: mins >= MIN_MINS ? "#00C853" : "#9146FF" }}>{mins}<span style={{ fontSize: 12, color: "#ADADB8" }}>/{MIN_MINS}</span></div>
            <div style={s.prog}><div style={{ height: "100%", borderRadius: 20, width: `${Math.min(100, mins / MIN_MINS * 100)}%`, background: mins >= MIN_MINS ? "#00C853" : "#9146FF", transition: "width .4s" }} /></div>
          </div>
        </div>
    ...
    <div style={s.app}>
      ...
      <div style={s.bar}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: state.liveActive ? "#00C853" : "#ADADB8" }}>
          ...
          {state.liveActive ? `Live — ${state.liveDate}` : "Offline"}
        </div>
      </div>
        ...
        {tab === "viewer" && <>
          <div style={s.card}>
            ...
            <div style={{ ...s.row, marginBottom: 12 }}>
              <input style={s.inp} placeholder="ex: xgamer123" value={nick} onChange={e => { setNick(e.target.value); const n = e.target.value.trim().toLowerCase(); setPreview(state.viewers[n] || null); }} onKeyDown={e => e.key === "Enter" && doRegister()} />
              <button style={s.btn()} onClick={doRegister}>Cadastrar</button>
            </div>
            ...
            <div style={{ fontSize: 12, color: state.liveActive ? "#00C853" : "#ADADB8", marginBottom: 8 }}>
              {state.liveActive ? "● Live ativa! Faça seu check-in." : "Nenhuma live ativa no momento."}
            </div>
            <div style={s.row}>
              ...
              <button style={s.btn("#00C853")} onClick={doCheckin}>Check-in</button>
            </div>
          </div>
        ...
        {tab === "streamer" && <>
          {!state.streamerUnlocked ? (
            <div style={{ ...s.card, maxWidth: 340, margin: "40px auto" }}>
              ...
              <div style={{ fontSize: 11, color: "#ADADB8", marginTop: 8 }}>Padrão: streamer123</div>
            </div>
          ) : <>
            <div style={s.grid2}>
              <div style={s.card}>
                <div style={{ fontWeight: 700, marginBottom: 10 }}>Status da live</div>
                <div style={{ fontSize: 12, color: "#ADADB8", marginBottom: 12 }}>{state.liveActive ? `Aberta desde ${state.liveDate}` : "Offline"}</div>
                <button style={{ ...s.btn(state.liveActive ? "#FF4747" : "#00C853"), width: "100%" }} onClick={toggleLive}>
              ...
              <div style={s.card}>
                ...
                <button style={{ ...s.btn(), width: "100%" }} onClick={drawWinner}>🎲 Sortear</button>
              </div>
            </div>
            ...
            {state.winner && (
              <div style={s.winner}>
                ...
                <button style={{ ...s.btnGhost, marginTop: 12, fontSize: 12, padding: "5px 12px" }} onClick={() => setState(s => ({ ...s, winner: null }))}>Limpar</button>
              </div>
            )}
                  ...
                  <div key={v.nick} style={{ borderBottom: "1px solid #26262C22", padding: "9px 0", display: "grid", gridTemplateColumns: "1fr 56px 56px 80px auto", gap: 8, alignItems: "center", fontSize: 12 }}>
                    ...
                    <div style={{ display: "flex", gap: 4 }}>
                      {state.liveActive && hasToday ? [15,30,60].map(m => (
                        <button key={m} style={{ ...s.btnGhost, padding: "3px 7px", fontSize: 11 }} onClick={() => addTime(v.nick, m)}>+{m}</button>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button style={{ ...s.btnGhost, color: "#FF4747", borderColor: "#FF474744", flex: 1 }} onClick={resetAll}>Resetar tudo</button>
              <button style={s.btnGhost} onClick={() => setState(s => ({ ...s, streamerUnlocked: false }))}>Sair</button>
            </div>
          </>}
        </>}
      </div>
      ...
      {flashMsg && (
        <div style={{ ...s.flash, background: flashColor, color: "#fff", opacity: flashMsg ? 1 : 0, transition: "opacity .3s" }}>{flashMsg}</div>
      )}
    </div>
  );
}
