...
export default async function handler(req, res) {
  ...
  if (msgType === 'notification') {
    ...
    // Chat message: handle #tailung command
    if (subType === 'channel.chat.message' && event) {
    ...
    // New sub or resub: mark viewer as subscriber
    if ((subType === 'channel.subscribe' || subType === 'channel.subscription.message') && event) {
      const userId = event.user_id;
      try {
        const state = await kv.get(STATE_KEY);
        if (state?.viewers?.[userId]) {
          state.viewers[userId].hasSub = true;
          await kv.set(STATE_KEY, state);
        }
      } catch (e) {
